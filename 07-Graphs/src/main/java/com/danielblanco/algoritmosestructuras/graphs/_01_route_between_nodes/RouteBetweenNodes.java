...
public class RouteBetweenNodes {
  ...
  public boolean isRouteBetween(Graph g, GraphNode start, GraphNode end) {
    ...
    Queue<GraphNode> cola = new LinkedList<>();
    cola.add(start);

    while (!cola.isEmpty()){
      GraphNode next = cola.remove();
      if (end == next) return true;

      for (GraphNode gn : next.adjacents.values()) {
        if (!gn.status.equals(GraphNodeStatus.Visited)){
          cola.add(gn);
          gn.status = GraphNodeStatus.Visited;
        }
      }
    }
    return false;
